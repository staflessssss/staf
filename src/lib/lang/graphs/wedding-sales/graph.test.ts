...
test("wedding sales graph rechecks a selected suggested date after unavailable availability", async () => {
  const result = await invokeWeddingSalesGraph({
    channel: "instagram",
    agentId: "agent-1",
    runtimeMode: "unified_v2",
  ...
  assert.match(result.turnToolObservations[0]?.result ?? "", /2026-10-18/);
...
test("instagram wedding sales uses Florida start price for known Florida leads", async () => {
  ...
  assert.match(result.responseDraft ?? "", /\$2,950/);
  assert.doesNotMatch(result.responseDraft ?? "", /\$3,490/);
  assert.match(result.responseDraft ?? "", /8-hour collections/i);
});
